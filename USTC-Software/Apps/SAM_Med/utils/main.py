...
parser.add_argument(
    "--img_path",
    type=str,
    default="./SAM-Med2D/data_demo/images/amos_0507_31.png",
...
class SamEncoder:
    ...
    def __call__(self, img: np.array, *args: Any, **kwds: Any) -> Any:
        ...
        # start_memory = GPUtil.getGPUs()[0].memoryUsed
        result = self._extract_feature(img)
        # end_time = time.time()
        # end_memory = GPUtil.getGPUs()[0].memoryUsed
...
class SamDecoder:
    ...
    def run(self,
        ...
        print(f"Memory used for inference: {memory_used} MB")
        '''
...
def main():
    ...
    #srh更改: 这边直接指定encoder_model, 不需要命令行参数输入,
